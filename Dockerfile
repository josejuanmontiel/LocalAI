...
RUN <<EOT bash
    if [ "${BUILD_TYPE}" = "intel" ]; then
        ...
        echo "deb [arch=amd64 signed-by=/usr/share/keyrings/intel-graphics.gpg] https://repositories.intel.com/gpu/ubuntu jammy/rolling unified" > /etc/apt/sources.list.d/intel-graphics.list
